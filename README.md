...
MongoDB is primary. If it is missing or cannot connect within three seconds, the app serves public content and images from `data/content-fallback.sqlite`. Connection failures during requests also switch to this snapshot. Fallback is read-only: sign-in, admin changes, bookings, registrations, and enquiries return 503 during the outage. The server retries MongoDB in the background every 30 seconds and automatically restores online services after a successful ping and database initialization. No restart is needed for a temporary network outage; changing hosting environment variables still requires a restart. The health endpoint reports `database: "sqlite"` and `readOnly: true`.
