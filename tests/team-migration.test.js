...
import { mongoStore } from '../apps/api/src/db/store.js';
...
for (const members of [
  ...
  test(`team migration preserves ${members.length ? 'custom profiles' : 'an intentionally empty team'} and does not rerun`, async () => {
    const store = await mongoStore(
      ...
      'gvk_test_' + randomUUID().replaceAll('-', '').slice(0, 24),
    );
    try {
      const about = {
        slug: 'about',
        ...
        body: 'Custom body',
        published: true,
        config: { team: { enabled: false, members } },
      };
      ...
      const pages = (await store.all('resources', { kind: 'pages' })).map((row) =>
        JSON.parse(row.data),
      );
